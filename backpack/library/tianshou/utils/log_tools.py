...
class BasicLogger(BaseLogger):
    ...
    def log_train_data(self, collect_result: dict, step: int) -> None:
        ...
        if collect_result["n/ep"] > 0:
            ...
            if step - self.last_log_train_step >= self.train_interval:
                ...
                self.write("train/rew_avg", step, collect_result["rew_avg"])
    ...
    def log_test_data(self, collect_result: dict, step: int) -> None:
        ...
        collect_result.update(rew=rew, rew_std=rew_std, rew_avg=rew_avg, len=len_, len_std=len_std)
        if step - self.last_log_test_step >= self.test_interval:
            self.write("test/rew", step, rew)
            self.write("test/len", step, len_)
            self.write("test/rew_avg", step, rew_avg)
