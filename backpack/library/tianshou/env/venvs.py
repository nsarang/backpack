...
class BaseVectorEnv(gym.Env):
    ...
    def __getattribute__(self, key: str) -> Any:
        ...
        """
        if key in ["metadata", "reward_range", "spec", "action_space", "observation_space"]:  # reserved keys in gym.Env
    ...
    def step(
        ...
        rew_stack, done_stack, info_stack = map(np.stack, [rew_list, done_list, info_list])
        if self.obs_rms and self.update_obs_rms:
    ...
    def save(self, id: Optional[Union[int, List[int], np.ndarray]] = None, **kwargs: Any) -> None:
        ...
        self._assert_is_not_closed()
        if self.is_async and len(self.waiting_id) > 0:
            raise RuntimeError(f"Environments {self.waiting_id} are still stepping, cannot " "save them now.")
        for j in id:
...
class RayVectorEnv(BaseVectorEnv):
    ...
    def __init__(self, env_fns: List[Callable[[], gym.Env]], **kwargs: Any) -> None:
        try:
            ...
            raise ImportError("Please install ray to support RayVectorEnv: pip install ray") from e
